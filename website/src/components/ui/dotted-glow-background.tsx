...
export const DottedGlowBackground = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Ellipse parameters — dots brightest at center, fading outward
      const cx = width * 0.5;
      const cy = height * 0.5;
      const rx = width * 0.9;
      const ry = height * 0.9;
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        // Full brightness at center, fades to 0 at ellipse edge
        const ellipseAlpha = Math.max(0, 1 - dist);

        if (ellipseAlpha <= 0) continue;

        const mod = (time * d.speed + d.phase) % 2;
        const lin = mod < 1 ? mod : 2 - mod;
        const a = (0.25 + 0.55 * lin) * ellipseAlpha;

        if (a > 0.4) {
          const glow = Math.min((a - 0.4) / 0.4, 1);
